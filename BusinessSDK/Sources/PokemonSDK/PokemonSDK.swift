...
private struct FetchPokemonListRequest: APIRequest {
...
private struct FetchPokemonDetailRequest: APIRequest {
...
public func fetchPokemonList(limit: Int = 20, offset: Int = 0) async throws -> PokemonListResponse {
    try await networkEngine.request(FetchPokemonListRequest(limit: limit, offset: offset))
}

public func fetchPokemonDetail(name: String) async throws -> PokemonDetailResponse {
    try await networkEngine.request(FetchPokemonDetailRequest(name: name))
}
