...
private class PokemonRepositoryFetchPokemonListRequest(private val limit: Int = 20, private val offset: Int = 0) : ApiRequest<PokemonListResponse> {
...
override suspend fun fetchPokemonDetail(name: String): PokemonDetailResponse {
    return networkEngine.request(PokemonRepositoryFetchPokemonDetailRequest(name = name))
}

}


interface PokemonSDKClientProtocol {
    val pokemonRepository: PokemonRepositoryProtocol
}

class PokemonSDKClient(
    networkEngine: NetworkEngine = NetworkEngine(baseUrl = PokemonSDKConfig.baseUrl)
) : PokemonSDKClientProtocol {
    override val pokemonRepository: PokemonRepositoryProtocol = PokemonRepository(networkEngine)
}
